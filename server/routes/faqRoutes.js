...
router.delete("/categories/:id", verifyToken, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      message: "ไม่สามารถลบหมวดหมู่ย่อยนี้ได้เนื่องจากมีคำถามใช้งานอยู่",
    });
  }
});
...
router.get("/faqs", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: rows.map((row) => ({ ...row, answer: sanitizeRichText(row.answer) })),
    });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
...
router.post("/faqs", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const finalAnswer = sanitizeRichText(answer);
    ...
    res.json({
      success: true,
      message: "บันทึกข้อคำถามสำเร็จ",
      insertId: result.insertId,
    });
  } catch (error) {
    res.status(500).json({
      success: false,
      message: "เกิดข้อผิดพลาดฝั่งฐานข้อมูลหลังบ้าน: " + error.message,
    });
  }
});
