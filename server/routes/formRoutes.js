...
router.get("/forms", async (req, res) => {
  try {
    ...
    const [rows] = await db.query(
      `SELECT id, title, description, theme, updated_at, status, clinic_type, form_type, publish_start_date, publish_end_date FROM forms`,
    );
    ...
    const formattedForms = rows.map((form) => {
      ...
      if (form.theme) {
        try {
          const themeData =
            typeof form.theme === "string"
              ? JSON.parse(form.theme)
              : form.theme;
          coverImage = themeData.headerImage || null;
        } catch (e) {
          console.error("Error parsing theme for form ID:", form.id);
        }
      }
...
router.post("/forms/:id/submit", async (req, res) => {
  ...
  try {
    ...
    const cleanIdentity = (identityValue || idFromAnswers || "").replace(
      /\D/g,
      "",
    );
    ...
    const [formRows] = await connection.query(
      "SELECT title, clinic_type, form_type FROM forms WHERE id = ?",
      [formId],
    );
    ...
    res
      .status(201)
      .json({ message: "บันทึกคำตอบสำเร็จ", responseId, masterCaseId });
  } catch (err) {
      ...
      rows.map(async (r) => {
        ...
        if (r.summary_data) {
          ...
          if (summary.citizenId)
            summary.citizenId = safeDecrypt(summary.citizenId);
...
router.patch("/forms/:id/clinic", verifyToken, async (req, res) => {
  try {
    await db.query("UPDATE forms SET clinic_type = ? WHERE id = ?", [
