...
router.get("/cases/:id/answers", verifyToken, async (req, res) => {
  try {
    ...
    const result = rows.map((r) => {
      ...
      if (
        r.question_title.includes("ชื่อ") ||
        r.question_title.includes("เบอร์") ||
        r.question_title.includes("โทร") ||
        r.question_title.includes("บัตร")
      ) {
        val = safeDecrypt(val);
      }
      return {
...
router.put("/master-cases/:id/close", verifyToken, async (req, res) => {
  try {
    ...
    await db.query("UPDATE mastercases SET status = 'Closed' WHERE id = ?", [
      masterCaseId,
    ]);
...
router.get("/cases/:id", verifyToken, async (req, res) => {
  try {
    const [rows] = await db.query("SELECT * FROM form_responses WHERE id = ?", [
      req.params.id,
    ]);
    if (rows.length === 0)
