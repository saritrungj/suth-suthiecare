...
const QuestionRenderer = ({
  ...
  useEffect(() => {
    ...
    const translateQuestion = async () => {
      ...
      if (q.options)
        tq.options = await Promise.all(
          q.options.map((opt) => translateTextSmart(opt)),
        );
      if (q.rows)
        tq.rows = await Promise.all(
          q.rows.map((row) => translateTextSmart(row)),
        );
      if (q.cols)
        tq.cols = await Promise.all(
          q.cols.map((col) => translateTextSmart(col)),
        );
  ...
  if (q.type === "group") {
    ...
    return (
      <div
        id={`question-${q.id}`}
        className="preview-group-wrapper"
        style={{ animationDelay: `${index * 0.05}s` }}
      >
        <div className="preview-group-header">
          <h3
            className="preview-sec__title"
            dangerouslySetInnerHTML={{ __html: displayTitle || "กลุ่มคำถาม" }}
          />
          {displayText && (
            <div
  ...
  return (
    <div
      id={`question-${q.id}`}
      className={`preview-sec ${hasError ? "preview-sec--error" : ""} ${q.isSubQuestion ? "preview-sec--sub" : ""}`}
      style={{ animationDelay: `${index * 0.05}s` }}
    >
        ...
        <div style={{ flex: 1 }}>
          <h3
            ...
            dangerouslySetInnerHTML={{
              __html: displayTitle || "คำถามที่ไม่มีชื่อ",
            }}
          />
          {q.required && <span className="req">*</span>}
          {q.hasDescription && displayText && (
            <div
              className="preview-hint"
              dangerouslySetInnerHTML={{ __html: displayText }}
            />
      ...
      <div className="preview-sec__body">
        ...
        {q.type === "file_upload" && (
          <div className="preview-file-upload-wrapper">
            ...
            {ans && ans.data && (
              <div
                className="preview-file-display"
              ...
              <div style={{ display: "flex", gap: "15px", flexWrap: "wrap" }}>
                <div style={{ flex: 1, minWidth: "120px" }}>
                  ...
                  <input
                    ...
                    value={bmiAns.weight || ""}
                    onChange={(e) =>
                      handleAnswer(q.id, { ...bmiAns, weight: e.target.value })
                    }
                  />
                </div>
                <div style={{ flex: 1, minWidth: "120px" }}>
                  ...
                  <input
                    ...
                    value={bmiAns.height || ""}
                    onChange={(e) =>
                      handleAnswer(q.id, { ...bmiAns, height: e.target.value })
                    }
                  />
                </div>
              </div>
        ...
        {q.type === "booking" && q.displayAs === "dropdown" && (
          ...
          >
            <option value="" disabled>
              {i18n.language === "en" ? "Select an option" : "เลือกคำตอบ"}
            </option>
            {q.options.map((opt, i) => {
              ...
              const optionLabel = isFull
                ? `[ ${fullText} ] - ${textOnly}`
                : isLimited
                  ? `[ ${leftText} ${remaining} ] - ${textOnly}`
                  : textOnly;
              return (
                <option key={i} value={originalTextOnly} disabled={isFull}>
                  {optionLabel}
                </option>
              );
            })}
                ...
                <div key={i} className="preview-option-wrapper">
                  ...
                  >
                    <input
                      ...
                      disabled={isFull}
                      onChange={() => {
                        if (!isFull) handleAnswer(q.id, opt);
                      }}
                    />
                    <span
                      style={{
        ...
        {(q.type === "dropdown" || q.type === "faculty") && (
          ...
          >
            ...
            {q.options.map((opt, i) => {
              ...
              const originalTextOnly = opt.replace(/<[^>]+>/g, "");
              return (
                <option key={i} value={originalTextOnly}>
                  {textOnly}
                </option>
              );
            })}
              ...
              <tbody>
                {q.rows.map((row, i) => (
                  ...
                  >
                    <td
                      dangerouslySetInnerHTML={{
                        __html: translatedQ?.rows?.[i] || row,
                      }}
                    />
                    {q.cols.map((col, j) => {
                      const key = String(i);
                      const rowAns =
                        ans?.[key] || (q.type === "grid_multiple" ? null : []);
                      const isChecked =
                        q.type === "grid_multiple"
                          ? rowAns === col
                          : (rowAns || []).includes(col);
                      return (
                        <td key={j} style={{ textAlign: "center" }}>
                          <input
                            type={
                              q.type === "grid_multiple" ? "radio" : "checkbox"
                            }
                            name={`grid-${q.id}-${i}`}
                            checked={isChecked}
                            onChange={() =>
