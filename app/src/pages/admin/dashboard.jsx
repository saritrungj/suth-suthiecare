  ...
  return (
    ...
    >
      ...
      <div className="stat-modern-header">
        ...
        <span
          className="stat-modern-label"
          style={{ color: urgent ? "#b91c1c" : "#475569", fontWeight: 700 }}
        >
          {label}
        </span>
      </div>
      {isLoading ? (
        ...
        <span
          className="stat-modern-value"
          style={{ color: urgent ? "#ef4444" : "#1e293b" }}
        >
          {value}
        </span>
      )}
...
const CustomDropdown = ({
  ...
  return (
    ...
    >
      ...
      <span className="db-select-value" style={textStyle}>
        {displayLabel}
      </span>
      <FiChevronDown
        className={`db-dropdown-icon ${isOpen ? "open" : ""}`}
        style={iconStyle}
      />
...
export default function Dashboard() {
    ...
    const loadInitialData = async () => {
      ...
      try {
        ...
        if (savedSettings?.last_selected_form_id) {
          ...
          const lastForm = formRes.data.find(
            (f) => String(f.id) === String(lastId),
          );
          ...
          if (lastForm) {
            ...
            if (lastForm.status === "published") {
              ...
              const parsedCharts =
                typeof savedSettings.active_charts === "string"
  ...
  useEffect(() => {
    if (filteredForms.length > 0) {
      const isStillInList = filteredForms.some(
    ...
    async (newChartData) => {
      try {
        ...
        const newChart = {
          ...
          question: newChartData.question,
          data: Array.isArray(res.data) ? res.data : res.data.data,
        };
        const updatedCharts = [
          ...(chartsByForm[selectedFormId] || []),
          ...
          <div className="main-dashboard-container">
            {selectedClinic === "sti" ? (
              <div className="sti-dashboard-wrapper">
                <div className="sti-hero-box">
                  <div>
                    ...
                    <div className="sti-hero-value">
                      {(masterCaseStats?.totalActive || 0) +
                        (masterCaseStats?.closed || 0)}
                    </div>
                  </div>
                  <div className="sti-hero-stats">
                    <div className="sti-hero-stat-item">
                      ...
                      <div className="value">
                        {masterCaseStats?.totalStudent || 0}
                      </div>
                    </div>
                    <div className="sti-hero-stat-item">
                      <div className="label">บุคคลทั่วไป</div>
                      <div className="value">
                        {masterCaseStats?.totalGeneral || 0}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="sti-stat-row">
                  ...
                  <StatBoxModern
                    isLoading={isStatsLoading}
                    icon={FiBriefcase}
                    label="เคสที่รอติดต่อกลับ"
                    value={(masterCaseStats?.waitingContact || 0)
                      .toString()
                      .padStart(2, "0")}
                    color="#f59e0b"
                  />
                  <StatBoxModern
                    isLoading={isStatsLoading}
                    icon={FiAlertCircle}
                    label="เคสเสี่ยงสูง (ฉุกเฉิน)"
                    value={(masterCaseStats?.highRisk || 0)
                      .toString()
                      .padStart(2, "0")}
                    color="#ef4444"
                    urgent
                ...
                <div className="sti-stat-row">
                  <div className="sti-blood-box">
                    <div>
                      ...
                      <div className="sti-blood-value">
                        {(masterCaseStats?.bloodTestNegative || 0) +
                          (masterCaseStats?.bloodTestPositive || 0)}
                      </div>
                    </div>
                  ...
                  <StatBoxModern
                    isLoading={isStatsLoading}
                    icon={FiPlusSquare}
                    label="รับยา PrEP (เจาะเลือดที่นี่)"
                    value={(masterCaseStats?.prepWithBlood || 0)
                      .toString()
                      .padStart(2, "0")}
                    color="#8b5cf6"
                  />
                  <StatBoxModern
                    isLoading={isStatsLoading}
                    icon={FiShield}
                    label="รับยา PrEP (มีผลเลือดมา)"
                    value={(masterCaseStats?.prepWithoutBlood || 0)
                      .toString()
                      .padStart(2, "0")}
                    color="#10b981"
                  />
                ...
                <div className="sti-stat-row">
                  <StatBoxModern
                    isLoading={isStatsLoading}
                    icon={FiArrowRight}
                    label="ส่งต่อ Safe Clinic"
                    value={(masterCaseStats?.forwardSafeClinic || 0)
                      .toString()
                      .padStart(2, "0")}
                    color="#ec4899"
                  />
                  <StatBoxModern
                    isLoading={isStatsLoading}
                    icon={FiArchive}
                    label="เคสที่ปิดแล้ว (Closed)"
                    value={(masterCaseStats?.closed || 0)
                      .toString()
                      .padStart(2, "0")}
                    color="#64748b"
                  />
              ...
              <div className="stats-grid">
                <StatBoxModern
                  isLoading={isStatsLoading}
                  icon={FiUsers}
                  label="เคสที่กำลังดูแล (Active)"
                  value={(masterCaseStats?.totalActive || 0)
                    .toString()
                    .padStart(2, "0")}
                  color="#14b8a6"
                />
            ...
            {filteredData.length === 0 ? (
              <div className="db-empty-state-card">
                ...
                >
                  ไม่มีข้อมูลสถิติในช่วงวันที่เลือก
                </h3>
                <p style={{ color: "#94A3B8", marginTop: "8px" }}>
                  กรุณาลองปรับเปลี่ยนช่วงเวลา หรือเลือกแบบฟอร์มใหม่อีกครั้ง
                </p>
              </div>
            ) : (
              <DndContext
                ...
                <button
                  ...
                  onClick={() =>
                    navigate("/admin/cases", {
                      state: { defaultFormId: selectedFormId },
                    })
                  }
                >
...
const SortableChart = React.memo(function SortableChart({
  id,
  title,
  type,
  question,
  ...
  const processedPieData = useMemo(() => {
    ...
    const questionDetails = formQuestions?.find(
      (q) => String(q.id) === String(question),
    );
    ...
    data.forEach((item) => {
      ...
      // 🟢 2. หากไม่มีใน result ให้ลองหาใน summary_data เผื่อไว้
        ...
        try {
          ...
          if (summary.score_results && Array.isArray(summary.score_results)) {
            const sr = summary.score_results.find(
              (s) =>
          ...
          if (!resultText) {
            resultText =
              summary[`${question}_label`] ||
              summary[`${question}_result`] ||
              "";
          }

          if (!resultText) {
            if (question === "food_1774859637605")
              resultText = summary.food_result_text;
            else if (question === "exercise_1774859637605")
              resultText = summary.exercise_result_text;
            else if (question === "emotion_1774859637605")
              resultText = summary.emotion_result_text;
          }
        } catch (e) {}
      }
      ...
      if (
        resultText &&
        resultText !== "-" &&
        !resultText.includes("รอผลสรุปคะแนน")
      ) {
        counts[resultText] =
          (counts[resultText] || 0) + Number(item.value || 1);
      }
    });
  ...
  return (
    ...
    >
      ...
      <h4 className="chart-title" style={{ marginTop: "10px" }}>
        {title}
      </h4>
            ...
            <ResponsiveContainer width="100%" height="100%">
              {type?.toLowerCase() === "pie" ? (
                <PieChart margin={{ top: 0, right: 35, bottom: 0, left: 35 }}>
                  ...
                  <Tooltip
                    formatter={(value, name) => [`${value} ราย`, name]}
                  />
                </PieChart>
              ) : (
                <BarChart
