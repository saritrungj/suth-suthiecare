    ...
    return (
      <div className="sfb-group-container">
        <p className="sfb-group-header">
          <FaLayerGroup size={16} color="#1967d2" /> คำถามย่อยในกลุ่มนี้ (
          {subs.length} ข้อ)
        </p>
        ...
        {subs.length > 0 ? (
          <div className="sfb-group-subs-wrapper">
            {visibleSubs.map((sq, idx) => {
              const visibleSqOptions = isActive
                ? sq.options
                : sq.options.slice(0, MAX_PREVIEW);
              const hiddenSqOptionsCount =
                sq.options.length - visibleSqOptions.length;
                ...
                <div key={sq.id} className="sfb-sub-question-card">
                  <div className="sfb-sub-question-header">
                    ...
                    <RichTextInput
                      ...
                      value={sq.title}
                      onChange={(val) =>
                        updateSubQuestion(q.id, sq.id, "title", val)
                      }
                      placeholder="พิมพ์คำถามย่อยที่นี่..."
                      showLists={false}
                    />
                    {isActive && (
                      <button
                        onClick={() => removeSubQuestion(q.id, sq.id)}
                        className="sfb-btn-remove-sub"
                      >
                        <FaTimes />
                      </button>
                    )}
                    ...
                    <div className="sfb-sub-options-wrapper">
                        ...
                        <div key={oIdx} className="sfb-sub-option-row">
                          ...
                          <div className="sfb-sub-opt-input-wrapper">
                            <input
                              ...
                              style={
                                sq.optionHasInput?.[oIdx]
                                  ? {
                                      flex: "none",
                                      width: `${Math.max(opt.length, 3) + 2}ch`,
                                      minWidth: "40px",
                                    }
                                  : {}
                              }
                          ...
                          {isActive && (
                            <>
                              ...
                              <button
                                onClick={() => {
                                  const currentHasInput = [
                                    ...(sq.optionHasInput ||
                                      Array(sq.options.length).fill(false)),
                                  ];
                                  currentHasInput[oIdx] =
                                    !currentHasInput[oIdx];
                                  updateSubQuestion(
                                    q.id,
                                    sq.id,
                              ...
                              <button
                                onClick={() => {
                                  const newOpts = sq.options.filter(
                                    (_, i) => i !== oIdx,
                                  );
                                  const newScores = (
                                    sq.optionScores || []
                      ...
                      {hiddenSqOptionsCount > 0 && !isActive && (
                        <div className="sfb-hidden-options-hint">
                          <FaPlus size={10} /> มีตัวเลือกอื่นอีก{" "}
                          {hiddenSqOptionsCount} รายการ...
                        </div>
                      )}
            ...
            {hiddenSubsCount > 0 && !isActive && (
              <div className="sfb-hidden-subs-hint">
                <FaPlus size={10} /> มีคำถามย่อยอื่นอีก {hiddenSubsCount} ข้อ...
                (คลิกการ์ดนี้เพื่อดูทั้งหมด)
              </div>
            )}
          </div>
        ) : (
        ...
        <div className="sfb-smooth-element sfb-sub-actions-footer">
          <button
            onClick={() => handleAddSub("multiple_choice")}
            className="sfb-btn-add-sub-type"
          >
            <FaRegCircle /> เพิ่มแบบหลายตัวเลือก
          </button>
          <button
            onClick={() => handleAddSub("checkboxes")}
            className="sfb-btn-add-sub-type"
          >
  ...
  if (q.type === "phone_number")
    return (
      <div className="sfb-dummy-input sfb-text-underlined sfb-dummy-phone">
        <FaPhoneAlt size={16} />
        <div>
          <div className="sfb-dummy-phone-hint">
            รูปแบบการกรอก: 0xx-xxx-xxxx (ตัวเลข 10 หลัก)
          </div>
          <div className="sfb-dummy-phone-sub">
            * ระบบจะใส่ขีดคั่นให้โดยอัตโนมัติเมื่อผู้ใช้พิมพ์
          </div>
        </div>
      </div>
    );
  ...
  if (q.type === "national_id")
    return (
      <div className="sfb-dummy-input sfb-text-underlined sfb-dummy-id">
        ...
        <div>
          <div className="sfb-dummy-phone-hint">
            รูปแบบช่องกรอก: x-xxxx-xxxxx-xx-x
          </div>
          <div className="sfb-dummy-phone-sub">
            * กล่องยินยอม PDPA จะแสดงอัตโนมัติในหน้าผู้กรอกฟอร์ม
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="sfb-video-container">
        {isActive && (
          <div style={{ marginBottom: "15px" }}>
            <input
              ...
              value={q.videoUrl || ""}
              onChange={(e) =>
                updateVideoUrl && updateVideoUrl(q.id, e.target.value)
              }
              style={{
                width: "100%",
                padding: "10px",
        ...
        {embedUrl ? (
          ...
          >
            {isActive
              ? "กรุณาวางลิ้งก์ YouTube เพื่อแสดงตัวอย่างวิดีโอ"
              : "วิดีโอ (ไม่มีลิ้งก์)"}
          </div>
        )}
      </div>
    ...
    const scoreModeUI = isActive && q.isScored && (
      <div className="sfb-smooth-element sfb-grid-score-toggle">
        ...
        <select
          ...
          value={q.scoreMode || "column"}
          onChange={(e) => updateScoreMode(q.id, e.target.value)}
        >
      ...
      <>
        ...
        <div className="sfb-grid-setup-container">
          <div className="sfb-grid-section">
            ...
            {visibleRows.map((row, i) => (
              <div key={i} className="sfb-grid-item-row">
                ...
                <RichTextInput
                  className="sfb-opt-input sfb-editable"
                  value={row}
                  onChange={(val) => updateGridItem(q.id, "rows", i, val)}
                  showLists={false}
                />
                ...
                {isActive && (
                  <button
                    className="sfb-btn-remove-opt sfb-smooth-element"
                    onClick={() => removeGridItem(q.id, "rows", i)}
                  >
                    <FaTimes />
                  </button>
                )}
            ...
            {hiddenRowsCount > 0 && !isActive && (
              <div className="sfb-grid-item-row">
                <span className="sfb-idx-num sfb-muted"></span>
                <span className="sfb-hidden-options-main">
                  <FaPlus size={10} /> มีอีก {hiddenRowsCount} แถว...
                </span>
              </div>
            )}
            <div
              className="sfb-grid-item-row sfb-add-row-trigger sfb-smooth-element"
              onClick={() => addGridItem(q.id, "rows")}
            >
              <span className="sfb-idx-num sfb-muted">
                {q.rows.length + 1}.
              </span>
              <span className="sfb-add-opt-text">เพิ่มแถว</span>
            </div>
          </div>
          ...
          <div className="sfb-grid-section">
            ...
            {visibleCols.map((col, i) => (
              <div key={i} className="sfb-grid-item-row">
                ...
                <RichTextInput
                  className="sfb-opt-input sfb-editable"
                  value={col}
                  onChange={(val) => updateGridItem(q.id, "cols", i, val)}
                  showLists={false}
                />
                {isActive &&
                  q.isScored &&
                  (q.scoreMode || "column") === "column" && (
                    <input
                      type="number"
                      className="sfb-opt-score-input"
                      value={q.colScores?.[i] ?? 0}
                      onChange={(e) =>
                        updateGridColScore(
                          q.id,
                          i,
                ...
                {isActive && (
                  <button
                    className="sfb-btn-remove-opt sfb-smooth-element"
                    onClick={() => removeGridItem(q.id, "cols", i)}
                  >
                    <FaTimes />
                  </button>
                )}
            ...
            {hiddenColsCount > 0 && !isActive && (
              <div className="sfb-grid-item-row">
                <span className="sfb-idx-num sfb-muted"></span>
                <span className="sfb-hidden-options-main">
                  <FaPlus size={10} /> มีอีก {hiddenColsCount} คอลัมน์...
                </span>
              </div>
            )}
            <div
              className="sfb-grid-item-row sfb-add-row-trigger sfb-smooth-element"
              onClick={() => addGridItem(q.id, "cols")}
            >
              {q.type === "grid_multiple" ? (
        ...
        {isActive && q.isScored && q.scoreMode === "cell" && (
          <div className="sfb-scoring-accordion-wrapper sfb-smooth-element sfb-cell-scoring-wrapper">
            <button
              ...
              onClick={() => setGridScoreExpanded(!gridScoreExpanded)}
            >
              {gridScoreExpanded ? (
                <>
                  <FaChartBar /> ซ่อนตารางกำหนดคะแนน
                </>
              ) : (
                <>
                  <FaTable /> กำหนดคะแนนรายช่อง (Cell Scoring)
                </>
              )}
            </button>
            <div
              className={`sfb-scoring-rules-collapse ${gridScoreExpanded ? "open" : ""}`}
            >
                    ...
                    <tr key={rowIndex}>
                      ...
                      {q.cols.map((col, colIndex) => (
                        <td key={colIndex}>
                          <input
                            type="number"
                            className="sfb-cell-input"
                            value={
                              editingCell === `${rowIndex}-${colIndex}`
                                ? (q.cellScores?.[rowIndex]?.[colIndex] ?? "")
                                : (q.cellScores?.[rowIndex]?.[colIndex] ?? 0)
                            }
                            onFocus={() =>
  ...
  return (
    <>
      {isActive && q.type === "booking" && (
        <div
          className="sfb-smooth-element sfb-grid-score-toggle"
        ...
        >
          ...
          <select
            ...
            onChange={(e) =>
              handlers.updateDisplayAs &&
              handlers.updateDisplayAs(q.id, e.target.value)
            }
          >
      ...
      <div className="sfb-options-container">
        {visibleOptions.map((opt, i) => {
          const handleOptImgUpload = (e) => {
            ...
            if (file) {
              ...
              reader.onloadend = () =>
                updateOptionImage(q.id, i, reader.result);
          ...
          return (
            <div key={i} className="sfb-option-wrapper">
              <div className="sfb-option-row" style={{ flexWrap: "nowrap" }}>
                {(q.type === "multiple_choice" || q.type === "booking") && (
                  <FaRegCircle className="sfb-opt-icon" />
                )}
                {q.type === "checkboxes" && (
                  <FaRegCheckSquare className="sfb-opt-icon" />
                ...
                {isActive && q.type === "booking" && (
                  <input
                    type="number"
                    className="sfb-opt-score-input"
                    value={q.optionLimits?.[i] ?? ""}
                    onChange={(e) => updateOptionLimit(q.id, i, e.target.value)}
                    title="จำนวนโควต้า/ลิมิต"
                    placeholder="ลิมิตจำนวน"
                    style={{ minWidth: "90px" }}
                  />
                )}
                    ...
                    {(q.type === "multiple_choice" ||
                      ...
                      <>
                        <input
                          ...
                          id={`file-opt-${q.id}-${i}`}
                          hidden
                          onChange={handleOptImgUpload}
                        />
                        <button
                          className="sfb-btn-add-image-opt"
                          title="เพิ่มรูปภาพ"
                          onClick={() =>
                            document
                              .getElementById(`file-opt-${q.id}-${i}`)
                              .click()
                          }
                        >
                          <FaImage />
              ...
              {q.optionImages && q.optionImages[i] && (
                <div className="sfb-option-image-preview">
                  ...
                  {isActive && (
                    <button
                      className="sfb-btn-remove-image sfb-smooth-element"
                      title="ลบรูปภาพ"
        ...
        {hiddenOptionsCount > 0 && !isActive && (
          <div className="sfb-option-row">
            ...
            <span className="sfb-hidden-options-main">
              <FaPlus size={10} /> มีตัวเลือกอื่นอีก {hiddenOptionsCount}{" "}
              รายการ... (คลิกเพื่อแก้ไข)
            </span>
          </div>
        )}
