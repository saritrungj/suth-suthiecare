...
const QUESTION_TYPES = [
  ...
  },
  {
    ...
    options: [
      {
        value: "group",
        label: "กลุ่มคำถาม (จัดกลุ่ม+รวมคะแนน)",
        icon: <FaThList />,
      },
      { value: "grid_multiple", label: "ตารางหลายตัวเลือก", icon: <FaTh /> },
      {
        value: "grid_checkbox",
        label: "ตารางช่องทำเครื่องหมาย",
        icon: <FaTh />,
      },
    ],
  },
  ...
  },
  {
    ...
    options: [{ value: "bmi", label: "ระบบคำนวณ BMI", icon: <FaWeight /> }],
  },
];
...
const CustomTypeSelector = ({ value, onChange }) => {
  ...
  const selectedOpt = QUESTION_TYPES.flatMap((g) => g.options).find(
    (o) => o.value === value,
  );
  ...
  return (
    <div className="sfb-custom-select-container" ref={ref}>
      <div
        className={`sfb-custom-select-trigger ${isOpen ? "open" : ""}`}
        onClick={() => setIsOpen(!isOpen)}
      >
        <div className="sfb-custom-select-val">
          <span className="sfb-opt-icon-sm">
            {selectedOpt?.icon || <FaMinus />}
          </span>
          ...
          {QUESTION_TYPES.map((group, gIdx) => (
            <React.Fragment key={gIdx}>
              ...
              {group.options.map((opt) => (
                <div
                  key={opt.value}
                  className={`sfb-select-option ${value === opt.value ? "selected" : ""}`}
                  onClick={() => {
                    onChange(opt.value);
                    setIsOpen(false);
                  }}
...
const SortableQuestion = ({
  ...
  const handleRuleChange = (idx, field, value) => {
    ...
    newRules[idx] = {
      ...newRules[idx],
      [field]:
        field === "min" || field === "max" ? parseFloat(value) || 0 : value,
    };
  ...
  const renderProfileBadge = (type, isUserStatus) => {
    if (isUserStatus)
      return (
        <>
          <FaBriefcase /> สถานะ / อาชีพ (ใช้คัดกรองสถิติอัตโนมัติ)
        </>
      );
    switch (type) {
      case "national_id":
        return (
          <>
            <FaIdCard /> ฟิลด์ระบุตัวตน (PDPA อัตโนมัติ)
          </>
        );
      case "full_name":
        return (
          <>
            <FaUser /> ชื่อผู้รับบริการ
          </>
        );
      case "phone_number":
        return (
          <>
            <FaPhoneAlt /> เบอร์โทรศัพท์
          </>
        );
      case "faculty":
        return (
          <>
            <FaBuilding /> สำนักวิชา / หน่วยงาน
          </>
        );
      case "main_issue":
        return (
          <>
            <FaClipboardList /> ปัญหา/ความต้องการหลัก
          </>
        );
      case "bmi":
        return (
          <>
            <FaWeight /> คำนวณ BMI อัตโนมัติ
          </>
        );
      default:
        return null;
      ...
      >
        <div className="sfb-section-header-bar sfb-theme-bg">
          <div className="sfb-section-header-left">
            <div
              className="sfb-form-drag-handle-inline sfb-smooth-element"
              {...attributes}
              {...listeners}
            >
              <FaGripVertical />
            ...
            <div className="sfb-section-indicator">
              ส่วนที่ {sectionIndex} จาก {totalSections} :
            </div>
            <input
              type="text"
          ...
          <div
            className="sfb-section-menu-container sfb-smooth-element"
            ref={footerMenuRef}
          >
            <button
              className="sfb-btn-section-menu"
        ...
        <div className="sfb-section-body-content">
          <RichTextInput
            tagName="h2"
            className="sfb-input-title"
            placeholder="ส่วนที่ไม่มีชื่อ"
            value={q.title}
            onChange={(val) =>
              setQuestions(
                questions.map((item) =>
  ...
  return (
    ...
    >
      {q.isScored && (
        <div
          className="sfb-score-indicator-badge"
          title="ข้อนี้เปิดโหมดให้คะแนน"
        >
          <FaTrophy />
        </div>
      )}
      ...
      {(q.isEditable || q.type === "phone_number") &&
        q.type !== "national_id" && (
      ...
      <div
        className="sfb-form-drag-handle sfb-smooth-element"
        {...attributes}
        {...listeners}
      >
        <FaGripHorizontal />
      ...
      <div className="sfb-question-row">
        <div className="sfb-question-input-wrapper">
          ...
          {q.type !== "description" && (
            <button
              className="sfb-btn-add-image sfb-smooth-element"
              title="เพิ่มรูปภาพ"
              onClick={() => document.getElementById(`file-q-${q.id}`).click()}
            >
              <FaImage />
            </button>
          )}
        ...
        {q.type !== "description" && (
          <div className="sfb-type-selector-wrapper sfb-smooth-element">
            <CustomTypeSelector
              value={q.isUserStatus ? "user_status" : q.type}
              onChange={(val) => updateQuestionType(q.id, val)}
            />
          </div>
        )}
      </div>
      ...
      {q.isScored && (
        <div className="sfb-scoring-accordion-wrapper sfb-smooth-element">
          ...
          >
            {rulesExpanded ? (
              <>
                <FaChartBar /> ซ่อนการตั้งค่าเกณฑ์แปลผล
              </>
            ) : (
              <>
                <FaCog style={{ marginRight: "6px" }} /> ตั้งค่าเกณฑ์แปลผลลัพธ์
              </>
            )}
          </button>

          <div
            className={`sfb-scoring-rules-collapse ${rulesExpanded ? "open" : ""}`}
          >
            {(q.scoringRules || []).map((rule, idx) => (
              <div key={idx} className="sfb-scoring-rule-card">
                <button
                  className="sfb-btn-remove-rule"
                ...
                <div className="sfb-scoring-rule-row">
                  ...
                  <input
                    type="number"
                    step="0.01"
                    className="sfb-rule-score-input"
                    value={rule.min}
                    onChange={(e) =>
                      handleRuleChange(idx, "min", e.target.value)
                    }
                  />
                  ...
                  <input
                    ...
                    value={rule.max}
                    onChange={(e) =>
                      handleRuleChange(idx, "max", e.target.value)
                    }
                  />
                  <div className="sfb-rule-gap"></div>
                  <input
                    type="color"
                    className="sfb-rule-color-input"
                    value={rule.color}
                    onChange={(e) =>
                      handleRuleChange(idx, "color", e.target.value)
                    }
                    title="สีที่จะใช้แสดงผล"
                  />
                  <input
                    type="text"
                    className="sfb-rule-text-input"
                    placeholder="ข้อความแปลผล เช่น เสี่ยงสูง"
                    value={rule.label}
                    onChange={(e) =>
                      handleRuleChange(idx, "label", e.target.value)
                    }
                  />
                </div>
                <div className="sfb-scoring-rule-row">
                  <input
                    type="text"
                    className="sfb-rule-text-input sfb-rule-advice-input"
                    placeholder="คำแนะนำเพิ่มเติม (แสดงหรือไม่ก็ได้)"
                    value={rule.advice}
                    onChange={(e) =>
                      handleRuleChange(idx, "advice", e.target.value)
                    }
                  />
                </div>
              </div>
      ...
      <div className="sfb-card-footer sfb-smooth-element">
        ...
        <div className="sfb-question-settings-container" ref={footerMenuRef}>
          <button
            className="sfb-btn-question-settings"
            onClick={() => setFooterMenuOpen(!footerMenuOpen)}
            ...
            <div className="sfb-question-settings-dropdown">
              {q.type !== "description" && !q.isDescriptionBlock && (
                <label className="sfb-dropdown-toggle-item">
                  <input
                    type="checkbox"
                    checked={!!q.hasDescription}
                    onChange={(e) =>
                      updateDescriptionPresence(q.id, e.target.checked)
                    }
                  />
              ...
              {isScoreableType && (
                <label className="sfb-dropdown-toggle-item">
                  <input
                    type="checkbox"
                    checked={!!q.isScored}
                    onChange={(e) => updateIsScored(q.id, e.target.checked)}
                  />
                  <span
                    className={
                      q.isScored ? "sfb-dropdown-highlight-scored" : ""
                    }
                  >
                    เปิดโหมดให้คะแนน
                  </span>
              ...
              {isEditableConfigurable && q.type !== "national_id" && (
                <label
                  className={`sfb-dropdown-toggle-item ${q.type === "phone_number" ? "sfb-dropdown-item-disabled" : ""}`}
                >
                  <input
                    type="checkbox"
                    ...
                    onChange={(e) =>
                      updateIsEditable &&
                      updateIsEditable(q.id, e.target.checked)
                    }
                  />
                  <span
                    className={
                      q.isEditable || q.type === "phone_number"
                        ? "sfb-dropdown-highlight-editable"
                        : ""
                    }
                  >
                    {q.type === "phone_number"
                      ? "อนุญาตให้แก้ไข (บังคับเปิด)"
                      : "อนุญาตให้แก้ไขในประวัติ"}
                  </span>
                </label>
              )}
            </div>
