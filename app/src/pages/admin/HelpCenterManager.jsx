...
export default function HelpCenterManager() {
    ...
    try {
      if (editingCatId) {
        ...
        MySwal.fire({
          icon: "success",
          title: "สำเร็จ",
          text: "อัปเดตชื่อหมวดหมู่เรียบร้อย",
          timer: 1200,
        ...
        MySwal.fire({
          icon: "success",
          title: "สำเร็จ",
          text: "เพิ่มหมวดหมู่ย่อยเรียบร้อย",
          timer: 1200,
          showConfirmButton: false,
        });
      }
  ...
  const handleDeleteCategory = async (id, name) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        MySwal.fire(
          "ข้อผิดพลาด",
          "ไม่สามารถลบหมวดหมู่ย่อยนี้ได้เนื่องจากมีคำถามใช้งานอยู่",
          "error",
        );
      }
    }
  ...
  const handleCatStatusChange = async (cat, newStatus) => {
    ...
    try {
      ...
      MySwal.fire(
        "ข้อผิดพลาด",
        "ไม่สามารถเปลี่ยนสถานะหมวดหมู่ในฐานข้อมูลได้",
        "error",
      );
      fetchCategoriesForForm(catSelectedClinic);
    }
    ...
    if (faq) {
      ...
      if (actualClinicId) {
        try {
          ...
          console.error(
            "เกิดข้อผิดพลาดในการดึงหมวดหมู่ย่อยสำหรับฟอร์มแก้ไข:",
            err,
          );
          setCategories([]);
        }
  ...
  const handleSaveFaq = async (e) => {
    e.preventDefault();
    const currentClinicId = faqForm.clinic_id
      ? parseInt(faqForm.clinic_id)
      : null;
    const currentCategoryId = faqForm.category_id
      ? parseInt(faqForm.category_id)
      : null;
    ...
    if (
      ...
      MySwal.fire(
        "แจ้งเตือน",
        "กรุณากรอกข้อมูลคำถาม คำตอบ และเลือกคลินิกหลักให้ครบถ้วน",
        "warning",
      );
      return;
    }
    ...
    if (faqForm.is_homepage === 1) {
      const isTurningOn =
        !editingFaq || (editingFaq && editingFaq.is_homepage !== 1);
      if (isTurningOn && homepageFaqCount >= 6) {
        MySwal.fire(
          "โควตาเต็ม",
          "ขณะนี้มีคำถามแสดงบนหน้าแรกครบ 6 ข้อแล้ว (รวมทุกคลินิก) กรุณาปิดข้ออื่นก่อนเปิดข้อนี้ค่ะ",
          "warning",
        );
        return;
      }
  ...
  const handleDeleteFaq = async (id, title) => {
    const result = await MySwal.fire({
      ...
      text: `คุณต้องการลบคำถาม "${title}" ใช่หรือไม่?`,
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#d33",
      cancelButtonColor: "#3085d6",
  ...
  const handleToggleClinicHelpStatus = async (clinicId, currentStatus) => {
    try {
      ...
      MySwal.fire({
        ...
        text: "ไม่สามารถอัปเดตสถานะคลินิกได้",
      });
    }
  };
    ...
    try {
      for (let i = 0; i < updatedFaqs.length; i++) {
        ...
        if (item.display_order !== correctOrder) {
          let actualCategoryId = item.category_id
      ...
      MySwal.fire(
        "ข้อผิดพลาด",
        "การเชื่อมต่อขัดข้อง ไม่สามารถบันทึกลำดับได้",
        "error",
      );
      fetchFaqs();
    }
      ...
      <main className="hc-main">
        <header className="hc-header">
          <div className="hc-header-info">
            ...
            <p className="hc-header-hint">
              ตั้งค่าหมวดหมู่ย่อยและข้อคำถามที่พบบ่อยแยกตามแต่ละคลินิกบริการ
            </p>
          </div>
              ...
              <table className="hc-table">
                <thead>
                  <tr>
                    ...
                    <th className="hc-col-question" style={{ width: "25%" }}>
                      คำถาม
                    </th>
                    <th className="hc-col-clinic" style={{ width: "15%" }}>
                      คลินิกหลัก
                    </th>
                    <th className="hc-col-category" style={{ width: "15%" }}>
                      หมวดหมู่ย่อย
                    </th>
                    <th
                      className="hc-col-homepage"
                    ...
                    <th className="hc-col-actions" style={{ width: "8%" }}>
                      จัดการ
                    </th>
                  </tr>
                </thead>
                    ...
                    currentFaqs.map((faq, index) => {
                      return (
                        <tr
                          ...
                          style={{
                            cursor:
                              draggedFaqIndex === index ? "grabbing" : "grab",
                            opacity: draggedFaqIndex === index ? 0.4 : 1,
                            backgroundColor:
                              draggedFaqIndex === index ? "#f8fafc" : "inherit",
                            transition: "all 0.2s ease",
                          }}
                        >
                          <td style={{ textAlign: "center", color: "#94a3b8" }}>
                            <FaGripVertical
                              title="คลิกค้างเพื่อลากจัดเรียง"
                              size={16}
                            />
                          </td>
                          ...
                          <td className="hc-col-clinic">
                            {faq.clinic_name || "-"}
                          </td>
                          <td className="hc-col-category">
                            <span className="hc-badge-category">
                              {faq.category_name &&
                              faq.category_name.trim() !== ""
                                ? faq.category_name
                                : "คำถามทั่วไปประจำคลินิก"}
                            </span>
                          </td>
                          <td className="hc-col-homepage">
                            {faq.is_homepage === 1 ? (
                              <span className="hc-status-badge active">
                                หน้าแรก
                              </span>
                            ) : (
                              <span className="hc-status-badge inactive">
                                ทั่วไป
                              </span>
                            )}
                          </td>
                          ...
                          <td className="hc-col-actions">
                            <div className="hc-actions-wrapper">
                              ...
                              <button
                                type="button"
                                className="hc-btn-action-delete"
                                onClick={() =>
                                  handleDeleteFaq(faq.faq_id, faq.question)
                                }
                                title="ลบคำถาม"
                              >
                                <FaTrash />
              ...
              <div className="table-pagination-footer">
                ...
                <div className="pagination-controls">
                  ...
                  <span className="page-indicator">
                    หน้า{" "}
                    <strong className="page-highlight">{currentPage}</strong>{" "}
                    จาก {totalPages || 1}
                  </span>
                  <button
                    className="page-btn"
          ...
          <div className="hc-modal-overlay">
            <div className="hc-modal-content" style={{ maxWidth: "700px" }}>
              ...
              <div className="hc-form-group">
                <label>
                  เลือกคลินิกที่ต้องการจัดการ{" "}
                  <span className="hc-required">*</span>
                </label>
                <select
                  required
                  ...
                  style={{
                    backgroundColor: editingCatId ? "#f1f5f9" : "#ffffff",
                    cursor: editingCatId ? "not-allowed" : "pointer",
                  }}
                >
              ...
              {catSelectedClinic && (
                <>
                  <hr
                    style={{ margin: "20px 0", border: "0.5px solid #e2e8f0" }}
                  />
                  <form
                  ...
                  >
                    <div
                      className="hc-form-group"
                      style={{ margin: 0, flex: 1 }}
                    >
                      <label>
                        {editingCatId
                          ? "กำลังแก้ไขชื่อหมวดหมู่"
                          : "ชื่อหมวดหมู่ย่อยใหม่"}{" "}
                        <span className="hc-required">*</span>
                      </label>
                      <input
                        type="text"
                  ...
                  >
                    รายการหมวดหมู่ทั้งหมดในคลินิกนี้
                  </label>
                  <div
                    style={{
                      maxHeight: "250px",
                        ...
                        {categories.length > 0 ? (
                          categories.map((cat, idx) => (
                            <tr
                              ...
                              style={{
                                cursor:
                                  draggedCatIndex === idx ? "grabbing" : "grab",
                            ...
                            >
                              ...
                              <td
                                style={{
                                  ...
                                  fontSize: "14px",
                                  fontWeight:
                                    editingCatId === cat.id ? "700" : "normal",
                                  color:
                                    editingCatId === cat.id
                                      ? "#F47932"
                                      : "#475569",
                              ...
                              >
                                <select
                                  ...
                                  onChange={(e) =>
                                    handleCatStatusChange(cat, e.target.value)
                                  }
                                  style={{
                                    padding: "4px 8px",
                                    borderRadius: "6px",
                          ...
                          <tr>
                            ...
                            >
                              ยังไม่มีหมวดหมู่ย่อยในคลินิกนี้
                            </td>
                          </tr>
                        )}
                      </tbody>
            ...
            <div className="hc-modal-content">
              ...
              <form onSubmit={handleSaveFaq}>
                <div className="hc-form-row">
                  ...
                  <div className="hc-form-group">
                    <label>
                      เลือกคลินิกหลัก <span className="hc-required">*</span>
                    </label>
                    <select
                      name="clinic_id"
                  ...
                  <div className="hc-form-group">
                    ...
                    <div style={{ display: "flex", gap: "8px" }}>
                      ...
                      >
                        <option value="">
                          -- ไม่มีหมวดหมู่ย่อย (คำถามทั่วไปประจำคลินิก) --
                        </option>
                        {categories &&
                          categories.length > 0 &&
                          categories.map((cat) => (
                    ...
                    {!faqForm.clinic_id && (
                      <small style={{ color: "#ef4444" }}>
                        * กรุณาเลือกคลินิกหลักก่อนเพื่อโหลดหมวดหมู่ย่อย
                      </small>
                    )}
                  </div>
                </div>
                ...
                <div className="hc-form-row">
                  <div className="hc-form-group">
                    <label>
                      สถานะการแสดงผล <span className="hc-required">*</span>
                    </label>
                    <select
                      required
                      value={faqForm.status}
                      onChange={(e) =>
                        setFaqForm({ ...faqForm, status: e.target.value })
                      }
                    >
                      <option value="published">เผยแพร่</option>
                      <option value="draft">ร่าง</option>
                ...
                <div className="hc-form-group">
                  <label>
                    ข้อคำถาม (Question) <span className="hc-required">*</span>
                  </label>
                  <input
                    type="text"
                    placeholder="เช่น ต้องเตรียมบัตรอะไรมาบ้างในวันนัดครั้งแรก?"
                    required
                ...
                <div className="hc-form-group">
                  <label>
                    คำตอบ (Answer) <span className="hc-required">*</span>
                  </label>
                  <div
                    style={{
                      background: "#ffffff",
                    ...
                    <div className="hc-toggle-row">
                      <label className="hc-switch">
                        <input
                          ...
                          disabled={
                            homepageFaqCount >= 6 && faqForm.is_homepage !== 1
                          }
                          onChange={(e) =>
                            setFaqForm({
                              ...faqForm,
                              is_homepage: e.target.checked ? 1 : 0,
                            })
                          }
                        />
                        <span
                      ...
                      <span
                        className={`hc-toggle-label ${homepageFaqCount >= 6 && faqForm.is_homepage !== 1 ? "disabled" : ""}`}
                      >
                ...
                <div className="hc-modal-actions">
                  ...
                  <button type="submit" className="hc-btn-save">
                    บันทึกข้อมูล
                  </button>
                </div>
              </form>
        ...
        {isClinicModalOpen && (
          <div className="hc-modal-overlay">
            ...
            >
              ...
              <div
                className="hc-clinic-search-wrapper"
                style={{ position: "relative" }}
              >
                <span
                  style={{
                          ...
                          <td style={{ textAlign: "center" }}>
                            <label className="hc-switch">
                              <input
                                ...
                                onChange={() =>
                                  handleToggleClinicHelpStatus(
                                    clinic.id,
                                    clinic.show_in_help_center,
                                  )
                                }
                              />
