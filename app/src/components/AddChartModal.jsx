...
const AddChartModal = ({ isOpen, onClose, onSave, formId }) => {
  ...
  const processedPieData = React.useMemo(() => {
    ...
    const selectedQDetails = questions.find(
    ...
    chartData.forEach((item) => {
      ...
      // 🟢 2. หากไม่มีใน result ให้ลองหาใน summary_data เผื่อไว้
        ...
        try {
          ...
          if (!resultText) {
            resultText =
              summary[`${selectedQuestion}_label`] ||
              summary[`${selectedQuestion}_result`] ||
              "";
          }

          if (!resultText) {
            if (selectedQuestion === "food_1774859637605")
              resultText = summary.food_result_text;
            else if (selectedQuestion === "exercise_1774859637605")
              resultText = summary.exercise_result_text;
            else if (selectedQuestion === "emotion_1774859637605")
  ...
  useEffect(() => {
    ...
    const fetchQuestions = async () => {
      try {
        ...
        const choiceQuestions = questionsData.filter(
          (q) =>
  ...
  useEffect(() => {
    const loadPreviewChart = async () => {
      ...
      try {
        const res = await getChartData(formId, selectedQuestion, null, null);
            ...
            <div className="form-group">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (q) {
                    const text = q.question_text || q.label || q.title || "";
              ...
              {selectedQuestion && chartType?.toLowerCase() === "bar" && (
                ...
                >
                  <div style={{ width: "100%", height: "280px" }}>
                    {" "}
                    {/* เพิ่มความสูงเล็กน้อยเพื่อให้มีพื้นที่สำหรับตัวเลขหัวแท่ง */}
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart
                        data={processedPieData}
                        margin={{ top: 25, right: 30, left: 10, bottom: 60 }}
                      >
                        <CartesianGrid
                          strokeDasharray="3 3"
                          vertical={false}
