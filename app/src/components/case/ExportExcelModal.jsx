...
export default function ExportExcelModal({
  ...
  const allQuestions = useMemo(
    () =>
      questions.filter((q) => q.type !== "section" && q.type !== "description"),
    [questions],
  );
  ...
  const handleExport = async () => {
    ...
    data.forEach((row, index) => {
      ...
      selectedQuestions.forEach((q) => {
        ...
        if (
          ...
          q.rows.forEach((rowTitle, idx) => {
            const subAns =
              ans && typeof ans === "object" ? ans[rowTitle] : undefined;
          ...
          q.subQuestions.forEach((sq, idx) => {
            const subAns =
              ans && typeof ans === "object" ? ans[sq.title] : undefined;
            rowData[`${q.id}_${idx}`] = formatAnswer(subAns);
          });
        } else {
        ...
        if (q.isScored) {
          ...
          const scoreObj = scoreResults.find(
            (s) => stripHtml(s.title) === cleanTitle,
          );
          if (scoreObj) {
    ...
    saveAs(
      blob,
      `Export_${stripHtml(formTitle || "Data").replace(/\s+/g, "_")}_${new Date().getTime()}.xlsx`,
    );
    onClose();
  };
      ...
      <div className="eem-modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="eem-modal-body">
          <div className="eem-selection-controls">
            <button className="eem-text-btn" onClick={handleSelectAll}>
              เลือกทั้งหมด
            </button>
            <button className="eem-text-btn" onClick={handleDeselectAll}>
              ไม่เลือกเลย
            </button>
          </div>
        ...
        <div className="eem-modal-footer">
          <button className="eem-cancel-btn" onClick={onClose}>
            ยกเลิก
          </button>
          <button
            className="eem-export-btn"
