...
export default function CaseTable({
  ...
  const dynamicQuestions = questions.filter(
    (q) =>
      q.type !== "section" &&
      q.type !== "description" &&
      visibleColumns.includes(q.id),
  );
  const showMasterRiskCol =
    hasScoring ||
    data.some(
      (row) =>
        row.overall_risk || (row.summary_data && row.summary_data.overall_risk),
    );
  ...
  return (
    <div className="table-box">
      <div className="table-scroll-wrapper table-scroll-with-footer">
        <table
          className={`case-table ${viewMode === "form" ? "form-view-mode" : ""}`}
        >
          <thead>
            <tr>
              ...
              <th className="th-date">
                <FiClock />{" "}
                {viewMode === "form" ? "ส่งฟอร์มเมื่อ" : "อัปเดตล่าสุด"}
              </th>
              ...
              {viewMode === "master" ? (
                <>
                  ...
                  {showMasterRiskCol && (
                    <th className="th-risk">
                      <FiActivity /> ความเสี่ยงคลินิก
                    </th>
                  )}
                </>
              ) : (
          ...
          <tbody>
            {currentData.map((row, index) => {
              ...
              const dateObj = new Date(
                row.submitted_at || row.updated_at || row.createdAt,
              );
              ...
              let riskToDisplay =
                viewMode === "master"
                  ? calculatedRisk ||
                    ...
                    "ประเมินใหม่"
                  : calculatedRisk || row.risk_level || "ไม่ระบุ";
              const patientName =
              ...
              return (
                <tr key={row.id}>
                  ...
                  <td className="td-caseid">
                    {row.master_case_id
                      ? `MC-${String(row.master_case_id).padStart(4, "0")}`
                      : `CASE-${String(row.id).padStart(4, "0")}`}
                  </td>
                  <td>
                    <button
                      className="case-name-link"
                      onClick={() => onSelectCase(row)}
                      title="คลิกเพื่อเปิดแฟ้มประวัติการรักษา"
                    >
                      {patientName}
                    </button>
                  </td>
                  ...
                  {viewMode === "master" ? (
                    ...
                    <>
                      ...
                      {dynamicQuestions.map((q) => {
                        const answerVal = findAnswerFlexible(
                          rawAnswers,
                          q.title,
                        );
                        const answerStr = formatAnswer(answerVal);
                        const sResult = scoreResults.find(
                          (sr) =>
                            sr.title === stripHtml(q.title) ||
                            sr.question_id === q.id,
                        );
                        const hoverText = sResult
                          ? `ผลประเมิน: ${sResult.label} (${sResult.score} คะแนน)\nคำตอบที่เลือก: ${answerStr !== "-" ? answerStr : "ไม่ได้ตอบ"}`
                          : answerStr !== "-"
                            ? answerStr
                            : "ไม่ได้ตอบคำถามนี้";
                          ...
                          >
                            {sResult ? (
                              <div
                                ...
                                style={{
                                  backgroundColor: sResult.color
                                    ? `${sResult.color}15`
                                    : "#f1f5f9",
                                  border: `1px solid ${sResult.color || "#cbd5e1"}`,
                                }}
                              >
                                <div
                                  className="result-dot"
                                  style={{
                                    backgroundColor: sResult.color || "#64748b",
                                  }}
                                ></div>
                                <span className="truncate-text">
                                  {sResult.label} ({sResult.score})
                                </span>
                              </div>
                            ) : (
                              <span className="truncate-text dynamic-ans">
                                {answerStr.length > 40
        ...
        <div className="table-pagination-footer">
          ...
          <div className="pagination-controls">
            <button
              ...
              title="หน้าก่อนหน้า"
            >
              <FiChevronLeft size={22} className="page-icon" />
            </button>
            <span className="page-indicator">
              หน้า <strong className="page-highlight">{currentPage}</strong> จาก{" "}
              {totalPages || 1}
            </span>
            <button
              className="page-btn"
              ...
              title="หน้าถัดไป"
            >
              <FiChevronRight size={22} className="page-icon" />
            </button>
          </div>
        </div>
      </div>
